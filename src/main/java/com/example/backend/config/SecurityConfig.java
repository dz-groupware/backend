...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
            .headers().frameOptions().sameOrigin()// X-Frame-Options를 sameOrigin으로 설정합니다. 이를 통해 동일 출처에서만 iframe을 허용합니다.
            .and()
            .addFilter(corsFilter) // CORS 필터 추가
            .csrf().disable() // CSRF 보호 비활성화
            .sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS) // 세션을 사용하지 않도록 설정 (JWT 사용)
            .and()
...
//                                "/api/v1/login"
    ...
    public class CustomFilterConfigurer extends AbstractHttpConfigurer<CustomFilterConfigurer, HttpSecurity> {
        ...
        public void configure(HttpSecurity builder) throws Exception {
            ...
            builder
                    .addFilterBefore(new JwtAuthenticationFilter(jwtKey,authenticationManager,objectMapper), UsernamePasswordAuthenticationFilter.class)
                    .addFilterAfter(new JwtAuthorizationFilter(jwtKey,employeeMapper,objectMapper),UsernamePasswordAuthenticationFilter.class);
        }
    }
